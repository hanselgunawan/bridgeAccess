...
router.get("/product_categories/filter", function (req, res) {
    bridgeAccessModel.selectAllCategoryAndSubcategoryName("bridge_goodsph_products", function (catData) {
        bridgeAccessModel.findByFilter("bridge_goodsph_products", req.query.min, req.query.max, req.query.category.replace("\\", "\\\\"),
                                        req.query.subcategory.replace("\\", "\\\\"), req.query.search, function (filterData) {
            if(req.params.subCategorySearch === "")
            {
                res.redirect("/product_categories");
            }
            ...
            bridgeAccessModel.selectPriceRangeByFilter("bridge_goodsph_products", req.query.min, req.query.max, req.query.category.replace("\\", "\\\\"),
                                                        req.query.subcategory.replace("\\", "\\\\").replace(/%5C/g,"\\"), req.query.search, function (priceRangeData) {
                var obj = {
                    product: filterData,
                    category:catData,
                    categories: getCategory(catData),
                ...
                if(req.query.search !== undefined && req.query.category !== undefined && req.query.subcategory === undefine)
                {
                ...
                {
                    obj["categorySearchQuery"] = req.query.category;
                }
                else if(req.query.search === undefined && req.query.category === undefined && req.query.subcategory !== undefined)
                {
                    obj["subCategorySearchQuery"] = req.query.subcategory;
                }
                console.log("HAHAHAHA: " + req.query.category);
                res.render("product_categories", obj);
            });
        });
    });
